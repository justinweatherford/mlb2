...
echo [1/5] Polling actuals for %SLATE_DATE%...
python mlb_poller.py --sport mlb --once --date %SLATE_DATE%
if errorlevel 1 (echo FAILED: mlb_poller.py & pause & exit /b 1)

echo.
echo [2/5] Enriching pregame cards with actuals...
python pregame_actuals_enrichment.py
if errorlevel 1 (echo FAILED: pregame_actuals_enrichment.py & pause & exit /b 1)

echo.
echo [3/5] Updating probability calibration...
...
echo.
echo [5/5] Reconciling fill prices and outcomes for %SLATE_DATE%...
