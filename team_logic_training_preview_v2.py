...
OUT_DIR = Path("outputs") / "team_logic_training_preview_v2"
...
TEAM_COL_CANDIDATES = [
    "team_abbr", "team", "abbr", "team_code", "selected_team_abbr",
]

DATE_COL_CANDIDATES = [
    "game_date", "date", "as_of_date", "context_date", "snapshot_date",
]
...
METRIC_CANDIDATES = {
    ...
    # Patch v2: actual form columns seen/expected in mlb_team_context
    ...
    # Optional rating-style recent form fields if present.
    "scoring_form": [
        "scoring_form_rating",
        "recent_scoring_form_rating",
        "form_rating",
        "current_form_rating",
        "recent_form_rating",
    ],
...
def latest_file(patterns: list[str]) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(
            "No candidate outcome CSV found. Use --candidates PATH."
        )
...
def load_team_context(conn: sqlite3.Connection) -> tuple[dict, dict, dict, dict]:
    ...
    if not team_col:
        return {}, {}, {}, {"warning": "No team column detected", "cols": cols}
...
def infer_runs_needed_team(row: dict) -> float | None:
    line = as_float(row.get("resolved_line_value"))
    if line is None:
        line = as_float(row.get("market_line_value"))
    if line is None:
...
def summarize(rows: list[dict], group_cols: list[str]) -> list[dict]:
    ...
    for key, rs in groups.items():
        ...
        pnl_sum = risk_sum = spread_sum = entry_sum = 0.0
        pnl_count = risk_count = spread_count = entry_count = 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read-only team logic training preview v2 with L1/L5/L10.")
    ...
    for r in candidates:
        ...
        l1_rpg, l1_col = get_metric(ctx, "l1_rpg")
        l5_rpg, l5_col = get_metric(ctx, "l5_rpg")
        l10_rpg, l10_col = get_metric(ctx, "l10_rpg")
        scoring_form, scoring_form_col = get_metric(ctx, "scoring_form")
        ...
        row.update({
            ...
            "l1_rpg_metric": l1_rpg,
            "l1_rpg_col": l1_col,
            "l1_rpg_bucket": rpg_bucket(l1_rpg),

            "l5_rpg_metric": l5_rpg,
            "l5_rpg_col": l5_col,
            "l5_rpg_bucket": rpg_bucket(l5_rpg),

            "l10_rpg_metric": l10_rpg,
            "l10_rpg_col": l10_col,
            "l10_rpg_bucket": rpg_bucket(l10_rpg),

            "scoring_form_metric": scoring_form,
            "scoring_form_col": scoring_form_col,
            "scoring_form_bucket": rating_bucket(scoring_form),
    ...
    write_csv(OUT_DIR / "enriched_candidate_team_context_v2.csv", enriched, base_fields)
    ...
    summaries = {
        ...
        "summary_by_l1_rpg_bucket.csv": ["l1_rpg_bucket"],
        "summary_by_l5_rpg_bucket.csv": ["l5_rpg_bucket"],
        "summary_by_l10_rpg_bucket.csv": ["l10_rpg_bucket"],
        "summary_by_scoring_form_bucket.csv": ["scoring_form_bucket"],
        "summary_by_runs_needed_team_bucket.csv": ["runs_needed_team_bucket"],
        "summary_by_candidate_type_and_l1_rpg.csv": ["candidate_type", "l1_rpg_bucket"],
        "summary_by_candidate_type_and_l5_rpg.csv": ["candidate_type", "l5_rpg_bucket"],
        "summary_by_candidate_type_and_l10_rpg.csv": ["candidate_type", "l10_rpg_bucket"],
        "summary_by_candidate_type_and_scoring_form.csv": ["candidate_type", "scoring_form_bucket"],
        "summary_by_candidate_type_and_runs_needed_team.csv": ["candidate_type", "runs_needed_team_bucket"],
        "summary_by_candidate_type_and_bullpen_risk.csv": ["candidate_type", "bullpen_risk_bucket"],
    }
    ...
    summary_lines = []
    summary_lines.append("# Team Logic Training Preview v2")
    ...
    sections = [
        ...
        ("Bullpen risk bucket", ["bullpen_risk_bucket"]),
        ("L1 RPG bucket", ["l1_rpg_bucket"]),
        ("L5 RPG bucket", ["l5_rpg_bucket"]),
        ("L10 RPG bucket", ["l10_rpg_bucket"]),
        ("Scoring form bucket", ["scoring_form_bucket"]),
    ...
    detected_cols = sorted(set(
        c for metric_cols in METRIC_CANDIDATES.values() for c in metric_cols
        if ctx_meta and c in ctx_meta.get("cols", [])
    ))
    summary_lines.append("## Detected Team Context Columns Used")
    summary_lines.append("")
    ...
    summary_lines.append("")
    for filename in ["enriched_candidate_team_context_v2.csv", *summaries.keys()]:
        summary_lines.append(f"- {filename}")

    (OUT_DIR / "team_logic_summary_v2.md").write_text("\n".join(summary_lines), encoding="utf-8")

    print(f"WROTE: {OUT_DIR}")
    print(f"  {OUT_DIR / 'team_logic_summary_v2.md'}")
    print(f"  {OUT_DIR / 'enriched_candidate_team_context_v2.csv'}")
