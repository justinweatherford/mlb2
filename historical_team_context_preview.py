...
OUT_DIR = Path("outputs") / "historical_team_context_preview"
...
def load_final_games(conn: sqlite3.Connection, season: str) -> list[dict]:
    rows = conn.execute(
        """
        SELECT
            game_pk,
    ...
    for r in rows:
        ...
        away = str(r[2] or "").strip().upper()
        home = str(r[3] or "").strip().upper()
...
def build_context_row(game: dict, team: str, opponent: str, side: str, history: list[dict], league_avg_rpg: float) -> dict:
    ...
    # Conservative proxies. These are for research only, not production ratings.
    if games_played == 0:
        ...
        # Higher is better run prevention. This is not bullpen risk.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read-only historical rolling team context preview from mlb_games.")
    ...
    games = load_final_games(conn, str(args.season))
    ...
    for game_date in sorted(by_date.keys()):
        ...
        # Build context for every game on this date BEFORE applying any same-day results.
        for g in date_games:
            away = g["away_abbr"]
            home = g["home_abbr"]

            context_rows.append(build_context_row(
                g, away, home, "away", team_history[away], league_avg_rpg
            ))
            context_rows.append(build_context_row(
                g, home, away, "home", team_history[home], league_avg_rpg
            ))

        # Now update histories after all same-day contexts are built.
        for g in date_games:
            away = g["away_abbr"]
            home = g["home_abbr"]
    ...
    context_path = OUT_DIR / f"historical_team_context_{args.season}.csv"
    ...
    latest_path = OUT_DIR / f"team_context_latest_{args.season}.csv"
    write_csv(
        latest_path,
        latest_rows,
        [
    ...
    summary_path = OUT_DIR / f"historical_team_context_summary_{args.season}.md"
    with summary_path.open("w", encoding="utf-8") as f:
        f.write("# Historical Team Context Preview\n\n")
        f.write(f"Generated: {datetime.utcnow().isoformat()} UTC\n\n")
        f.write(f"- Season: {args.season}\n")
        f.write(f"- Final games loaded: {len(games):,}\n")
        ...
        f.write("\n## Interpretation\n\n")
        f.write("- This is baseball-truth context only.\n")
        ...
        f.write("- `run_prevention_proxy` is not bullpen risk. It is team-level run prevention form.\n")
