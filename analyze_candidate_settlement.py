...
def parse_line_from_ticker(ticker, market_type=None):
    if not ticker:
        return None
    ...
    # Examples:
    # KXMLBTEAMTOTAL-26JUN161840MIAPHI-MIA8 -> 8
    # KXMLBTEAMTOTAL-26JUN161840MIAPHI-PHI7 -> 7
    if market_type == "team_total" or "TEAMTOTAL" in ticker:
        m = re.search(r"-[A-Z]{2,4}(\d+(?:\.\d+)?)$", ticker)
        if m:
            return float(m.group(1))

    # Examples:
    # KXMLBTOTAL-26JUN162140BALSEA-9 -> 9
    # KXMLBF5TOTAL-26JUN162140BALSEA-2 -> 2
    if market_type in ("full_game_total", "f5_total") or "TOTAL" in ticker:
...
def parse_team_from_ticker(ticker, market_type=None):
    if not ticker:
        return None
    ...
    # Example:
    # KXMLBTEAMTOTAL-26JUN161840MIAPHI-MIA8 -> MIA
    if market_type == "team_total" or "TEAMTOTAL" in ticker:
        m = re.search(r"-([A-Z]{2,4})\d+(?:\.\d+)?$", ticker)
        if m:
            return m.group(1)
...
def latest_market_inferred_settlement(conn, ticker):
    """
    Debug only. This does NOT drive settlement grading anymore.
    Final grading should come from MLB final score / inning score data.
    """
    if not ticker:
        return None
...
def selected_team(candidate, market):
    ...
    ticker = candidate.get("market_ticker") or (market or {}).get("market_ticker")
    market_type = candidate.get("market_type") or (market or {}).get("market_type")
    parsed_team = parse_team_from_ticker(ticker, market_type)

    return parsed_team.upper() if parsed_team else None


def line_value(candidate, market):
    candidate_line = safe_float(candidate.get("line_value"))
    if candidate_line is not None:
        return candidate_line

    market_line = safe_float((market or {}).get("line_value"))
    ...
    return parse_line_from_ticker(ticker, market_type)
...
def total_yes_wins(total_runs, line, yes_means):
    ...
    # Kalshi total/team-total tickers often encode integer thresholds.
    # Example: TEAMTOTAL...-MIA8 generally means "team scores 8 or more."
    if "under" in text:
...
def settle_candidate(candidate, game, market, f5, market_inferred):
    """
    Settlement grading is score-derived only.

    market_inferred is accepted only so the caller can still output debug columns,
    but it should not determine settled_yes_value.
    """
    market_type = candidate.get("market_type") or (market or {}).get("market_type")
    side = (candidate.get("side") or "YES").upper()
    ...
    if market_type not in SUPPORTED_TYPES:
        return unresolved("unsupported_market_type", f"unsupported market_type={market_type}")

    if not game:
        return unresolved("missing_game", "no matching mlb_games row")

    if safe_int(game.get("is_final")) != 1:
        return unresolved("game_not_final", f"mlb_games status={game.get('status')}")
    ...
    if market_type == "full_game_total":
        ...
        if not f5 or not f5.get("f5_has_all_5"):
            return unresolved(
                "missing_f5_score",
                "missing complete innings 1-5 in mlb_inning_scores",
            )
        ...
        if not team:
            return unresolved(
                "missing_selected_team",
                "moneyline requires selected_team_abbr",
            )
        ...
        if runs is None or opp is None:
            return unresolved(
                "missing_team_score",
                "could not map selected team to final score",
            )
        ...
        if not team or spread is None:
            return unresolved(
                "missing_spread_semantics",
                "spread requires selected_team_abbr and spread_value/line_value",
            )
        ...
        if not team or spread is None:
            return unresolved(
                "missing_spread_semantics",
                "f5 spread requires selected_team_abbr and spread_value/line_value",
            )
        ...
        if runs is None or opp is None:
            return unresolved(
                "missing_f5_team_score",
                "could not map selected team to f5 score",
            )
    ...
    if yes_wins is None:
        return unresolved(
            "unsettled_or_unknown",
            reason or "could not derive score-based settlement",
        )
...
def analyze_candidate(conn, candidate, date):
    ...
    resolved_line = line_value(candidate, market)
    resolved_team = selected_team(candidate, market)
    ...
    row = {
        ...
        # Keep both original and resolved fields so we can audit parser behavior.
        "candidate_line_value": candidate.get("line_value"),
        "resolved_line_value": resolved_line,
        "selected_team_abbr": resolved_team,
        ...
        # Debug only. These do not affect grading.
