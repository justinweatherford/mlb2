...
Total candidates: 159  |  Paper setups: 62

## 1. Candidate Mix

- `team_total`: 107 (67.3%)  observed=63  blocked=44  ← **dominant: 67.3% of all candidates exceeds 60% threshold**
- `fg_total`: 44 (27.7%)  observed=17  blocked=27
- `f5_total`: 8 (5.0%)  observed=4  blocked=4

## 2. Team Lag (team_total) Looseness
107 team_total candidates (67.3% of total).
...
Candidates with `baseline_source=first_discovery`: 159
Of those, with inflated market_mismatch_score (>0.5): 159
...
rally_still_active blocks: 55  validated=55  questionable=0  unclear=0
...
1. **Cap market_mismatch for first_discovery** — 159 candidate(s) have inflated scores from a zero/null open price.
2. **Tighten team_total triggers** — 67% of candidates are team_total; raise baseball_support threshold and require larger trailing gap.
