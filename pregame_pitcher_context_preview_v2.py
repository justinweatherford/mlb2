...
OUT_DIR = Path("outputs") / "pregame_pitcher_context_preview_v2"
...
def load_events(conn: sqlite3.Connection, games: dict) -> tuple[dict, dict]:
    ...
    for i in range(0, len(game_pks), chunk_size):
        ...
        for idx, r in enumerate(rows):
            ...
            raw_json = r[11] if len(r) > 11 else None

            raw_pitcher_id, raw_pitcher_name = pitcher_from_raw_json(raw_json)
...
def pitcher_from_raw_json(raw: Any) -> tuple[str, str]:
    # Best-effort parser. Safe fallback when MLB JSON stores pitcher person id/name in raw_json.
    if not raw:
        return "", ""
    try:
    ...
    for d in candidates:
        # Common MLB StatsAPI shapes:
        # matchup.pitcher.id/fullName, pitcher.id/fullName, details.pitcher, etc.
...
def aggregate_pitching_by_game_team(games: dict, events_by_game: dict) -> tuple[dict, dict]:
    ...
    for game_pk, game in games.items():
        ...
        for ev in sorted(events, key=lambda x: (x["inning"], str(x.get("event_time") or ""), x["event_index"])):
            ...
            pitcher_id = ev.get("pitcher_key") or ""
...
def main() -> None:
    ...
    md.append("- If no pitcher_id column exists, pitcher_name is used as a fallback key; raw_json is inspected for pitcher id/name when available.")
